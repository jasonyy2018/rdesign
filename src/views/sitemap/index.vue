      ...
      <div class="sitemap-section">
        ...
        <div class="templates-grid">
          <template v-for="template in templates" :key="template.id">
            <router-link
              :to="`/resume?category=${template.id}`"
              class="template-item"
              :title="`AI职升姬 简历模板 - ${template.title}`"
            >
              <span class="link-text">{{ template.title }}</span>
            </router-link>
          </template>
        </div>
      </div>
...
<script setup lang="ts">
  ...
  const sitePages = ref([
    { title: '首页', url: '/' },
    { title: '简历定制服务', url: '/resumeService' },
    { title: '在线制作模版列表', url: '/resume' },
    { title: 'AI简历智能生成', url: '/generateAiResume' },
    { title: '职场提升与简历优化指南', url: '/seo-articles' },
    { title: 'AI简历智能诊断', url: '/aiDiagnosticCV' },
    { title: 'word简历模版列表', url: '/word' },
    { title: 'ppt模版列表', url: '/ppt' },
    { title: '整站部署方案', url: '/webcode' },
    { title: '组织入驻', url: '/orgSettled' },
    { title: '项目文档', url: '/deployDoc' },
    { title: '会员开通', url: '/membership' },
    { title: '友链申请', url: '/linksApply' }
  ]);
